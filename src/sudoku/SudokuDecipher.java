 ...
 * @author Aziz
 * @version 7/25/2010
 * 		6/10/2010: Fixed exploitChainsInArray(): @see Line 870.
 * 		7/25/2010: getDecisiveString now uses StringBuilder to create its String.
 * 
 */
...
public class SudokuDecipher extends SudokuBoard {
	...
	private boolean isValidArray(ArrayList<Integer>[] array) {
		for (int k = 0; k < array.length; k++) {
			if (array[k].size() != 1 || array[k].get(0).intValue() == 0)
				continue;
			for (int j = k+1; j < array.length; j++)
				if (array[j].size() == 1 && array[j].get(0).intValue() != 0
						&& array[j].get(0).intValue() == array[k].get(0).intValue())
					return false;
		}
		return true;
	}
	 ...
	 * 		exists, we can eliminate those (the N indecisive numbers) numbers from the other cells. 
		...
		for (int n = 2; n <= 4; n++) {
			...
			if (cells.size() > n) {
				//System.out.printf("\tCells size is greater than n (%d > %d)%n", cells.size(), n);
					...
					if (n > 2) {
						//System.out.printf("\t\t@ n > 2 phase. n = %d; Representation = %d; whichOne = %d%n",n,representation,whichOne);
						//Iterating through the cells ArrayList<ArrayList<Integer>>.
						for (int k = 0; k < cells.size(); k++) {
							ArrayList<Integer> chainedNumbers = new ArrayList<Integer>();
							...
							if (alreadyChecked.contains(cells.get(k)))
								continue;
							//System.out.printf("\t\tWill be using %s from %s to compare to the rest of the array.%n",cells.get(k),cells);
							...
							if (cells.get(k).size() == n && chainedCells.size() == 0) {
								...
								//Iterate through the ArrayList<ArrayList<Integer>> again to compare cells.get(k) with all of them.
								for (int ki = 0; ki < cells.size(); ki++) {
									...
									//Check if all the numbers in cells.get(ki) are in chainedNumbers.
							...
							//Added "cells.size() >= n && " to line 871 because k would become greater than or equal to cells.size().
							if (cells.size() >= n && cells.get(k).size() < n && chainedCells.size() == 0) {
								chainedCells.add(cells.get(k));
								for (int c = 0; c < cells.get(k).size(); c++)
									chainedNumbers.add(cells.get(k).get(c));
	...
	private String getDecisiveString(ArrayList<Integer>[] array) {
		StringBuilder decisiveString = new StringBuilder(10);
		for (int k = 0; k < array.length; k++)
			if (array[k].size() == 1)
				decisiveString.append(array[k].get(0));
		return decisiveString.toString();
	}
	/**
	 * 
